...
for dataset in dataset_list:
    ...
    for i in range(0,5):
        ...
        for depth in range(5,50,10):
            ...
            # prev = []
            # for k, v in points_under_each_cluster.items():
            #     prev.append(len(v))
            ...
            # Find the majority from full_x and test_index

            # next_ = []
            # for k, v in points_under_each_cluster.items():
            #     next_.append(len(v))

            # for i, j in zip(range(len(prev)), range(len(next_))):
            #     print(prev[i], next_[j])
            ...
            if len(delete) != 0:        
                for i in delete: 
                    del points_under_each_cluster[i]

            # for i,v in points_under_each_cluster.items():
            #     print(i, v,end="\n")
            ...
            # print(X_train[min_distance_list].shape, X_train[idx_min])
            # print(y_train[min_distance_list].shape, y_train[idx_min])

            majority_data_X = full_X[min_distance_list]
            minority_data_X = X_train[idx_min]
            majority_data_y = full_y[min_distance_list]
