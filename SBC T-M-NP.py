        ...
        for depth in range(5,50,10):
            ...
            for i in points_under_each_cluster.keys():
                ...
                if(len(this_cluster) > 0):
                    ...
                    X_maj.extend(full_X[selected_points])
                    y_maj.extend(full_y[selected_points])

            X_sampled = np.concatenate((full_X[idx_min], np.array(X_maj)))
            y_sampled = np.concatenate((full_y[idx_min], np.array(y_maj)))
